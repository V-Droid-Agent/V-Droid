...
from html_representation.html_representation import turn_tree_to_html_input
...
class M3A(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str, step_idx: int, save_dir: str) -> base_agent.AgentInteractionResult:
    step_data = {
        'raw_screenshot': None,
    ...
    logging.warning('----------step ' + str(len(self.history) + 1))
    # pdb.set_trace()
    ...
    step_data['raw_ui_state'] = state
    step_data['logical_screen_size'] = logical_screen_size
    step_data['physical_frame_boundary'] = physical_frame_boundary
    ...
    infer.store_screen(step_data, step_idx, save_dir)
    

    if self.input_type == "html":
      html_descrption = turn_tree_to_html_input(state.forest)
    ...
    if self.input_type == "html":
      after_html_descrption = turn_tree_to_html_input(state.forest)
      summary_prompt = _summarize_prompt(
          goal,
          action,
          reason,
          html_descrption,
          after_html_descrption,
      )
