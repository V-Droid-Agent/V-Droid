...
class EventLog(object):
    ...
    def stop_profiling(self, output_dir=None):
        ...
        try:
            ...
            self.device.adb.shell(
                ["am", "profile", "stop", str(self.profiling_pid)])
            ...
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            event_trace_local_path = "%s/event_trace_%s.trace" % (
                output_dir, self.tag)
            self.device.pull_file(self.trace_remote_file,
                                  event_trace_local_path)
...
class UIEvent(InputEvent):
    ...
    def get_xy(x, y, view):
        if x and y:
            return x, y
        if view:
            from .autodroid_repsentation import DeviceState
...
class SwipeEvent(UIEvent):
    ...
    def send(self, device):
        ...
        end_x, end_y = UIEvent.get_xy(
            x=self.end_x, y=self.end_y, view=self.end_view)
    ...
    def get_event_str(self, state):
        if self.start_view is not None:
            ...
            start_view_str = "state=%s, start_x=%s, start_y=%s" % (
                state.state_str, self.start_x, self.start_y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
            raise InvalidEventException(msg)
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(state=%s, x=%s, y=%s, direction=%s)" %\
                   (self.__class__.__name__, state.state_str,
